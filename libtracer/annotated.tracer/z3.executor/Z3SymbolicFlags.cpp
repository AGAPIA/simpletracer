...
Z3_ast Z3FlagSF::Eval() {
	...
	PRINTF_SYM("<sym> lazySF: extract [%p] <= start[%d] size[%d] source[%p]\n",
			res, msb, 1, source);
	return res;
}
...
Z3_ast Z3FlagOF::Eval() {
	...
	PRINTF_SYM("<sym> lazyOF %p <= source[%p], p0[%p] p1[%p]\n", res, source, p[0], p[1]);
	return res;
}
